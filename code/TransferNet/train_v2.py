...
save_dir='./save_dir/Chinese_v2'
os.makedirs(save_dir,exist_ok=True)

logger=logging.getLogger('train_v2')
logger.setLevel(logging.INFO)

fh=logging.FileHandler(os.path.join(save_dir,'log_v2.txt'))
...
vis=Visdom(env='train_v2.py',log_to_filename=os.path.join(save_dir,'vis_log'))
...
def main():
    ...
    for k, v in vars(args).items():
        logger.info(k+':'+str(v))
        args_config[k]=v
    args_config['loss']='MSELoss()(last_e,answers)'
